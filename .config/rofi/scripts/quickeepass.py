...
def ask_choice(choices, default_filter):
    ...
    return rofi(f'column -t -s"\|" -c 0 | rofi -dmenu -p URL {Config.rofi_choice} {Config.rofi_userargs} -filter {default_filter}', stdin='\n'.join(choices))
...
def quickeeepass(args):
    ...
    choices = [ f"{e.url}|{e.username}|{e.password}" for e in kp.entries ]
